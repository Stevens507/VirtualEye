...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		(Xi, Yi, Xf, Yf) = box
		(concentrado,disgust,fear,happy,neutral,sad,surprise) = pred


		label = ''
		# Se agrega la probabilidad en el label de la imagen
		label = "{}: {:.0f}%".format(classes[np.argmax(pred)], max(concentrado,disgust,fear,happy,neutral,sad,surprise) * 100)
